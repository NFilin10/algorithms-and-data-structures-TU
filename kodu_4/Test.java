...
class Test {
    ...
    public static void sort(long[] jrj) {
        ...
        põimi(kimbud, jrj);
        System.out.println(Arrays.toString(jrj));

        List<ArrayList<Long>> kimbud1 = new ArrayList<>(32);

        for (int i = 0; i < 32; i++) {
            kimbud1.add(new ArrayList<>());
        }

        //jagame isikukoodid kimpudesse (jagamine toimub aasta alusel)
        for (long isikukood : jrj) {
            int kuu = (int) ((isikukood / 1000000) % 100);
            kimbud1.get(kuu).add(isikukood);
        }
    ...
    private static long[] põimi(List<ArrayList<Long>> kimbud, long[] jrj){
        int indeks = 0;
        for (int i = 0; i < kimbud.size(); i++) {
    ...
    private static long[] põimi1(List<ArrayList<Long>> kimbud, long[] jrj) {
        int indeks = jrj.length - 1;
        for (int i = kimbud.size() - 1; i >= 0; i--) {
            ArrayList<Long> kimp = kimbud.get(i);
            for (int j = kimp.size() - 1; j >= 0; j--) {
                jrj[indeks--] = kimp.get(j);
            }
        }
        return jrj;
    }
    ...
    public static void main(String[] args) {
        ...
        long[] a = {30001010004L, 10001010013L};
        sort(a);
